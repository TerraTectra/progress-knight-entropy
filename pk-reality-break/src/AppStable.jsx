...
import { OBSERVER_COMMANDS, OBSERVER_UPGRADES, makeSimulacrum, observerEffects, observerPointGain, observerUpgradeCost, personalityById, talentById, tickSimulacrum } from "./observer.js";
...
function UnlockHint({ item, ctx, language, title }) {
  if (!item) return null;
  return <div className="unlock-hint"><div className="unlock-hint-title">{title}: <b>{tr(item.name, language)}</b></div><div className="unlock-hint-req">Требование: {reqText(item.req, ctx, tr, language)}</div></div>;
}
...
function AchievementsView({ state, doneIds, language, selectedStage, setSelectedStage, onlyOpen, setOnlyOpen }) {
  ...
  return <div className="achievements-view">
    <Section title="Достижения">
      <div className="achievement-total-card"><div className="achievement-total-row"><span>Общий прогресс</span><b>{doneIds.length}/{MILESTONES.length}</b></div><ProgressBar value={pct(doneIds.length, MILESTONES.length)} /></div>
      <div className="achievement-controls"><label className="achievement-check"><input type="checkbox" checked={onlyOpen} onChange={(e) => setOnlyOpen(e.target.checked)} />Показывать только невыполненные</label></div>
    ...
    {activeStage && <Section title={`${stageIcon(activeStage.id)} ${achievementGroupLabel(activeStage.id, language)}`} tone="achievement-stage-panel">
      <div className="achievement-current-stage"><div><span>Текущий этап</span><b>{achievementGroupLabel(activeStage.id, language)}</b></div><div className="achievement-current-score"><b>{stageDone}/{allInStage.length}</b><span>выполнено</span></div></div>
      ...
      <div className="achievement-grid concept-grid">
        ...
        {shown.map((item) => {
          ...
          return <article key={item.id} className={`achievement-card concept-card ${done ? "done" : "locked"}`}><div className="achievement-card-head"><span className="achievement-status">{done ? "✓" : `${Math.floor(percent)}%`}</span><div className="achievement-title-wrap"><h3>{achievementName(item, language)}</h3><p>{achievementDesc(item, language)}</p></div></div><div className="achievement-progress-line"><span>Прогресс</span><b>{done ? "Готово" : `${fmt(progress.current)} / ${fmt(progress.target)}`}</b></div><ProgressBar value={percent} /><div className="achievement-reward"><span>Награда</span><b>{achievementReward(item, language)}</b></div></article>;
        })}
      </div>
    </Section>}
  </div>;
...
export default function AppStable() {
  ...
  const effects = useMemo(() => {
    ...
    return {
      ...
      jobXp: bonus.allXp * meta.allXp * uMods.jobXp * combineSkillBonuses(skillEffectMultiplier("Productivity", s.skillState), darkLearning) * itemMul(s.misc, "Work gloves") * itemMul(s.misc, "Personal squire") * itemMul(s.misc, "Entropy map") * (s.ownedPerks.includes("shadowDiscipline") ? 1.25 : 1) * (s.ownedPerks.includes("demonicAutomation") && s.autoPromote ? 1.5 : 1),
      skillXp: bonus.allXp * bonus.skillXp * meta.allXp * uMods.skillXp * combineSkillBonuses(skillEffectMultiplier("Concentration", s.skillState), darkLearning) * itemMul(s.misc, "Book") * itemMul(s.misc, "Research notes") * itemMul(s.misc, "Study desk") * itemMul(s.misc, "Library") * itemMul(s.misc, "Entropy map") * (s.ownedPerks.includes("shadowDiscipline") ? 1.25 : 1) * (s.ownedPerks.includes("demonicAutomation") && s.autoLearn ? 1.5 : 1),
  ...
  return <div className="app"><header><h1>Progress Knight: Reality Break</h1><div>x{speed.toFixed(2)} · U{s.universe} · {fmt(s.metaverse)} MP</div></header><nav>{tabs.map((item) => <button key={item} className={s.tab === item ? "active" : ""} onClick={() => patch({ tab: item })}>{tr(item, s.language)}</button>)}</nav><main><aside>
    ...
    {s.tab === "Evil Perks" && <Section title="Перки зла" tone="tone-dark">{EVIL_PERKS.map((perk) => <button key={perk.id} className="perk" disabled={s.ownedPerks.includes(perk.id) || s.evil < perk.cost || !reqUnlocked(perk.req, ctx)} onClick={() => buyPerk(perk)}><b>{tr(perk.name, s.language)}</b><span>{s.ownedPerks.includes(perk.id) ? "Куплено" : `${perk.cost} Evil`}</span><small>{perk.effect}</small></button>)}</Section>}
    {s.tab === "Multiverse" && <div className="grid2"><Section title="Мультивселенная" tone="tone-multiverse"><StatBox label="MP" value={fmt(s.metaverse)} /><button onClick={collapseUniverse}>Схлопнуть (+{fmt(universePointGain(s, effects))} MP)</button>{observerReady && <button className="final-perk" onClick={() => patch({ metaverse: s.metaverse - OBSERVER_UNLOCK_COST, observerUnlocked: true, tab: "Observer", simulacra: s.simulacra.length ? s.simulacra : [makeSimulacrum(0, observerEffects(s.observerUpgrades), true)] })}>Нечитаемый перк</button>}</Section><Section title="Вселенные" tone="tone-multiverse">{UNIVERSES.map((info) => <div className="universe-card" key={info.id}><b>{info.name}</b><small>{info.rule}</small>{info.id <= s.highestUniverse ? <button onClick={() => resetLife(false, info.id)}>Войти</button> : <button disabled={s.metaverse < info.unlockCost || info.id > s.highestUniverse + 1} onClick={() => unlockUniverse(info)}>Открыть · {fmt(info.unlockCost)} MP</button>}</div>)}</Section><Section title="Глобальные улучшения" tone="tone-multiverse">{META_UPGRADES.map((up) => { const level = s.metaUpgrades[up.id] || 0; const cost = metaUpgradeCost(up, level); return <button className="upgrade" key={up.id} disabled={s.metaverse < cost} onClick={() => buyMeta(up)}><b>{up.labels[Math.min(s.universe - 1, up.labels.length - 1)]}</b><span>lvl {level} · {fmt(cost)} MP</span><small>{up.effect}</small></button>; })}</Section></div>}
    {s.tab === "Observer" && <div className="grid2"><Section title="Наблюдатель" tone="tone-observer"><StatBox label="OP" value={fmt(s.observerPoints)} /><StatBox label="OP/sec" value={fmt(observerPointGain(s.simulacra, s.observerUpgrades, effects.observerGain, s.observerCommand))} /><select value={s.observerCommand} onChange={(e) => patch({ observerCommand: e.target.value })}>{OBSERVER_COMMANDS.map((cmd) => <option key={cmd.id} value={cmd.id}>{cmd.name}</option>)}</select><button onClick={buySim}>Купить симулякра · {s.simulacra.length === 0 ? 0 : fmt(Math.floor(25 * Math.pow(1.85, s.simulacra.length)))} OP</button></Section><Section title="Симулякры" tone="tone-observer">{s.simulacra.map((sim) => <div className="sim-card" key={sim.id}><b>{sim.name}</b><span>{talentById(sim.talentId).rarity}: {talentById(sim.talentId).name}</span><small>{personalityById(sim.personalityId).name} · lvl {sim.level} · U{sim.universe} · серия {sim.noMistakeStreak}</small><small>{sim.status}</small></div>)}</Section><Section title="Улучшения">{OBSERVER_UPGRADES.map((up) => { const level = s.observerUpgrades[up.id] || 0; const cost = observerUpgradeCost(up, level); return <button className="upgrade" key={up.id} disabled={s.observerPoints < cost} onClick={() => buyObserverUpgrade(up)}><b>{up.name}</b><span>lvl {level} · {fmt(cost)} OP</span><small>{up.effect}</small></button>; })}</Section></div>}
    {s.tab === "Settings" && <Section title="Настройки"><select value={s.language} onChange={(e) => patch({ language: e.target.value })}>{LANGUAGES.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button onClick={() => patch({ paused: !s.paused })}>{s.paused ? "Продолжить" : "Пауза"}</button><button className="danger" onClick={() => { clearSave(); setS(defaultSave()); }}>Полный сброс</button>{CONTACTS.map((c) => <div className="contact" key={c.label}><b>{c.label}</b><span>{c.value}</span></div>)}<div className="support">По вопросам сотрудничества или добровольной поддержки пишите в Telegram @tahioff или на почту luter.rouze@gmail.com.</div></Section>}
    {s.tab === "Admin Console" && <Section title="Админ-консоль"><div className="admin-buttons">{ADMIN_SPEED_MULTIPLIERS.map((m) => <button key={m} className={s.adminSpeedMultiplier === m ? "active" : ""} onClick={() => patch({ adminSpeedMultiplier: s.adminSpeedMultiplier === m ? 1 : m })}>x{m}</button>)}</div><button onClick={() => patch({ ownedPerks: Array.from(new Set([...s.ownedPerks, "shadowDiscipline", "darkPatronage", "wickedBargain", "soulFurnace", "deathDefiance", "demonicAutomation", "realityBreak"])), highestUniverse: 10, universe: 10, metaverse: Math.max(s.metaverse, 2000000), tab: "Multiverse" })}>Open Universe X</button></Section>}
  </section></main></div>;
}
