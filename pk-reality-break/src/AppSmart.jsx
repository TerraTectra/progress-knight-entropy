...
function AchievementsView({ state, doneIds = [], language, selectedStage, setSelectedStage, onlyOpen, setOnlyOpen }) {
  ...
  return <div className="achievements-view">
    <Section title={tx("Achievements", language)}>
      ...
      <div className="achievement-controls"><label className="achievement-check"><input type="checkbox" checked={onlyOpen} onChange={(e) => setOnlyOpen(e.target.checked)} />{tx("Show only incomplete", language)}</label></div>
    ...
    {activeStage && <Section title={`${stageIcon(activeStage.id)} ${achievementGroupLabel(activeStage.id, language)}`} tone="achievement-stage-panel">
      ...
      <div className="achievement-grid concept-grid">{shown.length === 0 && <div className="empty-achievements">{tx("All achievements in this stage are complete.", language)}</div>}{shown.map((item) => {
...
export default function AppSmart() {
  ...
  }, [s.skillState, s.autoLearn, s.autoSkillMode, currentAutoPlan?.skill?.name]);
  useEffect(() => {
  ...
  return <div className="app"><header><h1>Progress Knight: Reality Break</h1><div>x{speed.toFixed(2)} · U{s.universe} · {fmt(s.metaverse)} MP</div></header><nav>{tabs.map((item) => <button key={item} className={s.tab === item ? "active" : ""} onClick={() => patch({ tab: item })}>{tx(item, s.language)}</button>)}</nav><main><aside>
    ...
    <Section title={tx("Automation", s.language)}><CheckboxRow label={tx("Auto job", s.language)} checked={s.autoPromote} onChange={(v) => patch({ autoPromote: v })} /><div className="muted">{tx(s.autoJobBranch, s.language)} · {getLevel(s.jobState, s.jobName)}/{nextTen(getLevel(s.jobState, s.jobName))}</div><CheckboxRow label={tx("Auto skill", s.language)} checked={s.autoLearn} onChange={(v) => patch({ autoLearn: v })} /><label className="field-row"><span>{tx("Auto skill mode", s.language)}</span><select value={s.autoSkillMode || "smart"} onChange={(e) => patch({ autoSkillMode: e.target.value })}>{AUTO_SKILL_MODES.map((mode) => <option key={mode.id} value={mode.id}>{tx(mode.label, s.language)}</option>)}</select></label><div className="muted">{tx(s.skillName, s.language)} · {getLevel(s.skillState, s.skillName)}/{currentAutoPlan?.target || nextTen(getLevel(s.skillState, s.skillName))}</div><CheckboxRow label={tx("Time warp", s.language)} checked={s.warp} onChange={(v) => patch({ warp: v })} /><CheckboxRow label={tx("Auto shop", s.language)} checked={s.autoShop} disabled={!autoShopOpen} onChange={(v) => patch({ autoShop: v })} /></Section>
